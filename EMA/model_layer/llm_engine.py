...
class LLMEngine:
    def __init__(self, model_name: str, converstation_history: ConversationMemory = None):
        """
    ...
    def generate_response(self, prompt: str, tool_calls: list = None, max_length: int = 1024, temperature: float = 0.01, top_p: float = 0.9, top_k: int = 5):

        try:
            #Tokenize with padding, truncation, and explicit attention mask
            inputs = self.tokenizer(prompt, return_tensors="pt", padding=True, truncation=True, max_length=4096).to(self.device)
            ...
            with torch.inference_mode():
                output_ids = self.model.generate(
                    ...
                    max_new_tokens=512,  # Set to 1024 for longer responses
                    do_sample=True,
    ...
    async def generate_response_stream(self, user_message: str, tool_calls: list = None, max_length: int = 2048, temperature: float = 0.01, top_p: float = 0.9, top_k: int = 5):
        """
